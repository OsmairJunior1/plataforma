// =====================================================
//  VGR ACADEMY — DATABASE (localStorage)
...
// =====================================================
const RATE_LIMIT_KEY = 'vgracademy_login_attempts';
...
const DB_KEY = 'vgracademy_db';

// Senha admin padrão — hash SHA-256 de 'skillflix_salt_2026_admin123' (salt fixo, não alterar)
...
function DB_init() {
  ...
  const oldData = localStorage.getItem('vgracademy_admin') || localStorage.getItem('eduflix_admin');
...
const DB = {
  ...
  getSession: () => {
    try {
      const s = localStorage.getItem('vgracademy_session');
      if (!s) return null;
  ...
  setSession: (userId) => {
    ...
    localStorage.setItem('vgracademy_session', JSON.stringify({
      userId,
  ...
  clearSession: () => localStorage.removeItem('vgracademy_session'),
